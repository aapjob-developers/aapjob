...
class SelectRegisterOptionScreen extends StatefulWidget {
  SelectRegisterOptionScreen({Key? key}) : super(key: key);
  @override
  _SelectRegisterOptionScreenState createState() => new _SelectRegisterOptionScreenState();
}

class _SelectRegisterOptionScreenState extends State<SelectRegisterOptionScreen> {
  ...
  _hrsubmit() async {
    ...
    if(Provider.of<AuthProvider>(context, listen: false).getacctype()!="none")
    {
      await Provider.of<AuthProvider>(context, listen: false).checkaccount2("hr", Provider.of<AuthProvider>(context, listen: false).getMobile(),Provider.of<AuthProvider>(context, listen: false).getAccessToken(), route);
    }
    else
    {
  ...
  _candidatesubmit() async {
    ...
    if(Provider.of<AuthProvider>(context, listen: false).getacctype()!="none")
    {
      await Provider.of<AuthProvider>(context, listen: false).checkaccount2("candidate", Provider.of<AuthProvider>(context, listen: false).getMobile(),Provider.of<AuthProvider>(context, listen: false).getAccessToken(), route);
    }
    else
    {
      CommonFunctions.showSuccessToast(" Please Select an Account Type");
  ...
  route(bool isRoute, String route,String status, String errorMessage,String acctype) async {
    // _user.email
    setState(() {
