...
class HrSupportScreen extends StatefulWidget {
  HrSupportScreen({Key? key}) : super(key: key);
  @override
  _HrSupportScreenState createState() => new _HrSupportScreenState();
}
...
class _HrSupportScreenState extends State<HrSupportScreen> {
  ...
  getreasons() async {
    try {
      Response response = await _dio.get(_baseUrl + AppConstants.COMPLAINT_REASONS_URI+"?type=HR");
  ...
  SubmitComplaint(String dropdownvalue,String details) async {
    try {
      FormData formData = new FormData.fromMap({"reason": dropdownvalue, "details":details,"userid":Provider.of<AuthProvider>(context, listen: false).getUserid(),"usertype":"HR" });
